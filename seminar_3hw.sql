...
*/
select count(distinct city) 'Кол-во уникальных записей в столбце city' from customers;

-- 3. Напишите запрос, который выбрал бы наименьшую сумму для каждого заказчика.
select cnum, min(amt) from orders group by cnum; 

-- 4*. Напишите запрос, который бы выбирал заказчиков чьи имена начинаются с буквы C.
select * from customers where cname like 'C%';

-- 5. Напишите запрос, который выбрал бы высший рейтинг в каждом городе.
select city, max(rating) from customers group by city;

# (Задача с презинтации)
...
-- эксперимент (выводит все данные по минимальному заказу)
select * from orders where amt = (select min(amt) min_order from orders);
